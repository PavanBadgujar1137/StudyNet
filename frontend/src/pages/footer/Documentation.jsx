  ...
  const docCategories = [
    {
      ...
      items: [
        ...
        { title: 'Setting up your first course or program', snippet: 'Create self-paced or Circle-based programs with video modules, reflections, and resources.', details: 'Build structured 6-week containers or self-paced courses. Combine video lessons with reflection check-ins and downloadable workbooks.' }
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { title: 'Group sessions and Circles', snippet: 'Host interactive Circles with breakout rooms, group chat, and shared screen.', details: 'Host up to 8 participants per Circle with interactive screen sharing, chat moderation, and recording options.' },
    ...
    },
    {
      ...
      icon: FiLayers,
      desc: 'Build self-paced or live Circle curricula with structured modules and reflections.',
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        { title: 'Peer supervision groups', snippet: 'Join small, closed groups of 4-6 practitioners for monthly confidential case consultation.', details: 'Connect with experienced peers for monthly clinical case reviews in confidential closed groups.' },
        ...
        <div className="relative max-w-[1440px] mx-auto px-4 md:px-8">
          <div className="max-w-5xl mx-auto flex flex-col items-center justify-center text-center">
            ...
            <h1 className="text-center text-[clamp(20px,4.2vw,52px)] leading-tight max-w-full mx-auto my-4 font-black tracking-tight text-[#0F172A]">
              <span className="block whitespace-nowrap">Everything you need to run your practice</span>
              <span className="text-[#2563EB] block whitespace-nowrap">organized the way you actually work.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center items-center gap-2.5 max-w-5xl mx-auto mt-8 p-3 rounded-2xl bg-slate-50 border border-slate-200/80 shadow-inner">
              <button
                onClick={() => setActiveCategory('all')}
                className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 inline-flex items-center gap-2 ${
                  activeCategory === 'all' 
                    ? 'bg-[#2563EB] text-white shadow-md border border-blue-600 scale-[1.02]' 
                    : 'bg-white text-[#334155] hover:bg-blue-50 hover:text-[#2563EB] border border-slate-200 shadow-sm'
                }`}
              >
              ...
              {docCategories.map(cat => {
                ...
                return (
                  ...
                  >
                    <Icon className={isActive ? 'text-white' : 'text-[#2563EB]'} /> {cat.title}
                  </button>
                )
              })}
            </div>
      ...
      <section className="px-4 md:px-8 py-12 max-w-[1440px] mx-auto w-full space-y-10">
        {filteredCategories.length === 0 ? (
          ...
          filteredCategories.map(cat => {
            ...
            return (
              <div key={cat.id} className="p-6 sm:p-10 rounded-3xl bg-white border border-slate-200 shadow-sm transition-all duration-200">
                <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isCollapsed ? '' : 'pb-6 border-b border-slate-100 mb-6'}`}>
                  <div 
                    onClick={() => toggleCategoryCollapse(cat.id)}
                    className="flex items-center gap-4 cursor-pointer group"
                  >
                    <div className="w-12 h-12 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-[#2563EB] text-xl font-bold group-hover:bg-blue-50 transition-colors">
                      <CatIcon />
                    </div>
                    <div>
                      <h2 className="text-xl sm:text-2xl font-extrabold text-[#0F172A] tracking-tight group-hover:text-[#2563EB] transition-colors">{cat.title}</h2>
                      <p className="text-xs sm:text-sm text-[#64748B] font-medium">{cat.desc}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-3 self-start sm:self-auto">
                    <span className="text-xs font-mono font-bold px-3.5 py-1.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
                      {cat.items.length} Guides Available
                    </span>
                    <button
                      onClick={() => toggleCategoryCollapse(cat.id)}
                      className="px-3.5 py-1.5 rounded-full bg-[#2563EB] hover:bg-blue-700 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm"
                    >
                      {isCollapsed ? (
                        <><span>Expand</span> <FiChevronDown /></>
                      ) : (
                        <><span>Collapse</span> <FiChevronUp /></>
                      )}
                    </button>
                  </div>
                </div>
